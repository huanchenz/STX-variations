...
public:
    ...
    static const int leafslots = BTREE_MAX(8, 4096 / (sizeof(_Key) + sizeof(_Data))); //page size = 4K

    /// Number of slots in each inner node of the tree. Estimated so that each node
    /// has a size of about 256 bytes.
    //static const int innerslots = BTREE_MAX(8, 256 / (sizeof(_Key) + sizeof(void*))); //page size = 256
    static const int innerslots = BTREE_MAX(8, 4096 / (sizeof(_Key) + sizeof(void*))); //page size = 4K
    ...
    static const size_t binsearch_threshold = 256; //page size = 4K
};
...
public:
    ...
    private:
        ...
        /// The currently referenced leaf node of the tree
        typename btree::leaf_multidata_node * currnode;

        /// Current key/data slot referenced
        unsigned short currslot;
	unsigned short currdata_pos;
    ...
    public:
	  ...
	  : currnode(NULL), currslot(0), currdata_pos(0)
        { }

        /// Initializing-Constructor of a mutable iterator
        inline static_iterator(typename btree::leaf_multidata_node* l, unsigned short s)
	  : currnode(l), currslot(s), currdata_pos(0)
        { }

        inline leaf_multidata_node *get_currnode() {
          return currnode;
        }
        ...
        inline unsigned short get_currdata_pos() {
          return currdata_pos;
        }
        ...
        inline data_type & data() const
        {
            return currnode->slotdata[currslot][currdata_pos];
        }

        /// Prefix++ advance the iterator to the next slot
        inline static_iterator& operator ++ ()
        {
	  if (currdata_pos < currnode->slotdata_count[currslot] - 1) {
	    ++currdata_pos;
	  }
	  else {
            if (currslot + 1 < currnode->slotuse) {
	  ...
	  if (currdata_pos > 0) {
	    ...
	    currdata_pos = currnode->slotdata_count[currslot] - 1;
	  }
	  ...
	  if (currdata_pos > 0) {
        ...
        {
	  return (x.currnode == currnode) && (x.currslot == currslot) && (x.currdata_pos == currdata_pos);
        }
    ...
    public:
        ...
        inline hybrid_iterator()
          : currnode(NULL), currnode_static(NULL), currslot(0), currslot_static(0), currdata_pos_static(0), currtree(0)
        { }

        /// Initializing-Constructor of a mutable iterator
        inline hybrid_iterator(typename btree::leaf_node* l, short s, typename btree::leaf_multidata_node* l_static, short s_static, unsigned short ctree)
          : currnode(l), currslot(s), currnode_static(l_static), currslot_static(s_static), currdata_pos_static(0), currtree(ctree)
        { }
...
public:
    ...
    static_iterator find_static(const key_type& key)
    {
        ...
        int slot = find_lower(leaf, key);
        return (slot < leaf->slotuse && key_equal(key, leaf->slotkey[slot]))
               ? static_iterator(leaf, slot) : static_end();
    }
	...
	if ((m_stats.itemcount == 0) || !KeyMayMatch(reinterpret_cast<const char*>(&key), sizeof(key_type), bloom_filter)) {
	  ...
	  //return hybrid_iterator(m_tailleaf, m_tailleaf ? m_tailleaf->slotuse : 0, static_key_iter.get_currnode(), static_key_iter.get_currslot(), 1);
	  return hybrid_iterator(NULL, 0, static_key_iter.get_currnode(), static_key_iter.get_currslot(), 1);
	}
      }
	...
	return hybrid_iterator(NULL, 0, static_key_iter.get_currnode(), static_key_iter.get_currslot(), 1);
      ...
      static_key_iter = find_static(key);
      //return hybrid_iterator(key_iter.get_currnode(), key_iter.get_currslot(), m_tailleaf_static, m_tailleaf_static ? m_tailleaf_static->slotuse : 0, 0);
      return hybrid_iterator(key_iter.get_currnode(), key_iter.get_currslot(), static_key_iter.get_currnode(), static_key_iter.get_currslot(), 0);
    }
...
*/
    ...
    static_iterator lower_bound_static(const key_type& key)
    {
        ...
        int slot = find_lower(leaf, key);
        return static_iterator(leaf, slot);
    }
...
public:
	...
	if (key_lessequal(ln->slotkey[0], ln_static->slotkey[0])) {
	  ...
	  currkey = ln_static->slotkey[0];
	    ...
	    //std::cout << "currkey = " << currkey << "\n";
	    //std::cout << "data = " << data_buf[data_count-1] << "\n";
          }
          leaf_node *ln_temp = ln->nextleaf;
          free_node(ln);
          ln = ln_temp;
